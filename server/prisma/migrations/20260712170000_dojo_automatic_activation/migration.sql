ALTER TYPE "ProviderStatus" ADD VALUE IF NOT EXISTS 'active';
...
-- Preserve visibility for records that were already approved under the legacy
-- status vocabulary.
UPDATE public.dojos
SET status = 'active',
...
-- Backfill only complete legacy registrations. Rejected/suspended records are
-- excluded by the pending predicate, and missing proof/payment/profile fields
-- keep a record pending for manual investigation.
UPDATE public.dojos AS dojo
SET status = 'active',
    approved = TRUE,
    "approvedAt" = COALESCE(dojo."approvedAt", CURRENT_TIMESTAMP),
    verified = FALSE
WHERE dojo.status = 'pending'
  AND dojo.approved = FALSE
...
UPDATE public.dojos SET approved = FALSE WHERE status <> 'active' AND approved = TRUE;

ALTER TABLE public.dojos
  DROP CONSTRAINT IF EXISTS "Dojo_active_approved_consistency";
ALTER TABLE public.dojos
  ADD CONSTRAINT "Dojo_active_approved_consistency"
  CHECK ((status = 'active' AND approved = TRUE) OR (status <> 'active' AND approved = FALSE));

CREATE INDEX IF NOT EXISTS "dojos_status_approved_verified_category_idx"
