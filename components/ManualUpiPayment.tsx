    ...
    <section className={`rounded-2xl border border-acid/25 bg-acid/10 p-4 ${className}`}>
      <div className="flex flex-col gap-3">
        <div>
          ...
          <p className="mt-2 text-xs leading-5 text-zinc-300">Scan the QR or pay the exact amount to the receiver UPI ID.</p>
        </div>
        ...
        <div className="min-w-0 rounded-xl border border-white/10 bg-ink/70 p-3">
          ...
          <div className="mt-2 flex items-center gap-2">
            <code className="min-w-0 flex-1 overflow-x-auto whitespace-nowrap text-sm font-semibold text-white">{upiId}</code>
            <button
              type="button"
              onClick={copyUpiId}
      ...
      {showTransactionInput ? <label className="mt-4 block text-sm font-medium text-white">
