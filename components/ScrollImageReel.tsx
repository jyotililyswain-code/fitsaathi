...
export function ScrollImageReel() {
  ...
  return (
    <section
      ref={ref}
      className="relative overflow-hidden border-y border-white/10 bg-white/[0.03] py-12"
    >
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <p className="text-sm text-acid">Scroll motion gallery</p>
        <h2 className="mt-2 text-3xl font-bold text-white">
          Training energy that moves with you
        </h2>
      </div>
      <motion.div
        style={{ x, rotate }}
        className="mt-8 flex w-max gap-5 px-4 will-change-transform"
      >
        {images.map(([src, alt]) => (
          <div
            key={src}
            className="relative h-56 w-56 shrink-0 overflow-hidden rounded-2xl border border-white/10 bg-white"
          >
            <Image
              src={src}
              alt={alt}
