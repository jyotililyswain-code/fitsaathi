...
export default function ProductDirectory() {
  ...
  return <main className="mx-auto max-w-7xl px-4 py-10 sm:px-6 sm:py-14 lg:px-8">
    <section className="flex flex-col justify-between gap-6 rounded-3xl border border-white/10 bg-gradient-to-br from-white/[0.07] to-acid/[0.05] p-6 sm:p-8 lg:flex-row lg:items-end"><div><p className="text-sm font-medium text-acid">Fitness marketplace</p><h2 className="mt-2 text-3xl font-bold text-white sm:text-4xl">{category || "Equipment for your next milestone"}</h2><p className="mt-4 max-w-2xl leading-7 text-zinc-300">Trusted sellers rank first, followed by verified stores, ratings, and real sales.</p></div><button type="button" aria-expanded={filtersOpen} onClick={() => setFiltersOpen((value) => !value)} className="inline-flex w-fit items-center gap-2 rounded-full border border-white/15 px-4 py-2.5 text-sm text-white"><Filter className="h-4 w-4" />{filtersOpen ? "Hide filters" : "Filters"}</button></section>
