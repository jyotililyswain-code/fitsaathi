...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (action === "seller_status") {
      ...
      if (!canManageOrders(role) || !["pending", "paid", "processing", "shipped", "delivered", "cancelled", "refunded"].includes(value)) return NextResponse.json({ error: "Order management permission required." }, { status: 403 });
